  ...
  for (const item of array) {
    ...
    <div class="flex-shrink-0">
      <img src="./assets/images/${
        item.category
      }.png" style="width: 120px; height: 120px; border-radius: 20px;" alt="${
      item.category
    } icon">
    </div>
    <div class="flex-grow-1 ms-3">
      <h2 style="font-size: 24px; font-weight: bold; padding: 0px; margin: 0px">${
        item.favorite ? "⭐" : ""
      } ${item.title}</h2>
      <p><b>Category:</b> ${item.category.toUpperCase()}</p>
      <button class="btn btn-dark" id="watch--${
        item.videoId
      }">Watch Video</button>
    </div>
    <div>
...
const eventListeners = () => {
  ...
  document.querySelector("#filterContainer").addEventListener("click", (e) => {
    ...
    const filter = (filterString) => {
      const newDataArray = data.filter(
        (video) => video.category === filterString
      );
    ...
    if (e.target.id === "music") {
      ...
      const newDataArray = data.filter((video) => video.favorite);
      cardsOnDom(newDataArray);
  ...
  document.querySelector("#cardContainer").addEventListener("click", (e) => {
    ...
    if (e.target.id) {
      ...
      if (e.target.id.includes("watch")) {
        ...
        //videoPlayer(videoId); <-- alternative, look into differences
        document.location = "#";
      }
  ...
  form.addEventListener("submit", (e) => {
    ...
    // filter for querySelectors
    // const valueFilter = (filterString) => {
    //   const newDataArray = data.filter(
    //     (video) => video.category === filterString
    //   );
    //   cardsOnDom(newDataArray);
